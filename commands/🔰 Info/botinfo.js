...
module.exports = {
  ...
  category: "🔰 Info",
  description: "Отправляет подробную информацию о клиенте",
  usage: "botinfo",
  type: "bot",
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    let tempmsg = await message.reply({
      embeds: [new Discord.MessageEmbed().setColor(es.color)
        .setAuthor(client.la[ls].cmds.info.botinfo.loading, "https://cdn.discordapp.com/emojis/756773010123522058.gif")
      ]
    })
    cpuStat.usagePercent(function(e, percent, seconds) {
      ...
      const botinfo = new Discord.MessageEmbed()
        .setAuthor(client.user.tag + " Информация", es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL(), `https://discord.com/api/oauth2/authorize?client_id=${client.user.id}&permissions=8&scope=bot%20applications.commands`)

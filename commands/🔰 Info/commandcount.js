...
module.exports = {
  ...
  aliases: ["cmdcount"],
  usage: "commandcount",
  description: "Показывает количество команд",
  type: "bot",
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    let tempmsg = await message.reply({
      embeds: [new MessageEmbed()
        .setColor(es.color)
        .setAuthor(handlemsg(client.la[ls].cmds.info.commandcount.tempmsg), "https://cdn.discordapp.com/emojis/756773010123522058.gif")
      ]
    })
    let lines = 0
    let letters = 0
    ...
    await tempmsg.edit({
      embeds: [new MessageEmbed()
        ...
        }) + ` | **[\`${client.slashCommands.size}\`] Слеш комманд**`)

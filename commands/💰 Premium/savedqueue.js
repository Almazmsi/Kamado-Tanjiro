...
module.exports = {
  ...
  description: `Сохраняет текущую очередь с именем`,
...
**Name**:
> \`Может быть любым, максимум 10 букв.\`
**Options**:
> \`выберите трек, который вы хотите удалить\``,
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    ...
    switch (Type.toLowerCase()) {
      case `addcurrenttrack`: {
        ...
        return message.reply({
          embeds: [new MessageEmbed()
            .setTitle(`<a:yes:945004422172385330> Добавлено ${track.title} в очередь \`${Name}\``.substr(0, 256))
    ...
    case `removesong`: {
      ...
      return message.reply({
        embeds: [new MessageEmbed()
          .setTitle(`<a:yes:945004422172385330> Удалено ${deletetrack.title} из очереди \`${Name}\``.substr(0, 256))
    ...
    case `mix`: {
      ...
      return message.reply({
        embeds: [new MessageEmbed()
          .setTitle(`<a:yes:945004422172385330> Перемешано ${newtracks.length} треков в очереди \`${Name}\``.substr(0, 256))
    ...
    case `removeduplicates`: {
      ...
      return message.reply({
        embeds: [new MessageEmbed()
          .setTitle(`<a:yes:945004422172385330> Удалено ${counter} треков из очереди \`${Name}\``.substr(0, 256))
    ...
    case `list`: {
      ...
      //return susccess message
      return swap_pages(client, message, description, `Твои сохранённые очереди`)
    }
    break;
    case `create`:
    case `createsave`:
    ...
    case `details`: {
      ...
      return swap_pages(client, message, array, `Детальная информация про: \`${Name}\` [${tracks.length} треки]`)
    }
    break;
    default:
      return message.reply({

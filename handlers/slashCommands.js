  ...
  try {
    ...
    readdirSync("./slashCommands/").forEach((dir) => {
      ...
      if (pull.name && pull.description) {
        ...
        if (pull.options && pull.options.length > 0) {
          for (const option of pull.options) {
            if (option.User && option.User.name && option.User.description) {
              ...
              console.log(`Вариант отсутствует Название или/и Описание ${pull.name}`)
            }
          }
        }
        ...
        console.log(file, `ошибка -> отсутствует help.name, или help.name не является строкой.`.brightRed);
      }
    });
  } catch (e) {

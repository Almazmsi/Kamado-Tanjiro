...
module.exports = client => {
  ...
  try {
    client.logger(`Discord Bot теперь онлайн и готов к использованию!`)

    client.logger(
      `Имя бота: `.brightBlue + `${client.user.tag}`.blue + `\n` +
      `Сервер(а) `.brightBlue + `${client.guilds.cache.size} Servers`.blue + `\n` +
      `Смотрит за: `.brightBlue + `${client.guilds.cache.reduce((a, b) => a + b.memberCount, 0)} Members`.blue + `\n` +
      `Префикс: `.brightBlue + `${config.prefix}`.blue + `\n` +
      `Команды: `.brightBlue + `${client.commands.size}`.blue + `\n` +
      `Discord.js: `.brightBlue + `v${Discord.version}`.blue + `\n` +
      `Node.js: `.brightBlue + `${process.version}`.blue + `\n` +
      `Платформа: `.brightBlue + `${process.platform} ${process.arch}`.blue + `\n` +
      `Память: `.brightBlue + `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} MB / ${(process.memoryUsage().rss / 1024 / 1024).toFixed(2)} MB`.blue
    );

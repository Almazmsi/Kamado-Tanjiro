...
module.exports = async (client, interaction) => {
      if (interaction.isCommand()) {
        ...
        if (!guild) {
          return interaction.reply({
            content: ":x: Взаимодействие работает только внутри серверов!",
            ephemeral: true
          }).catch(() => {});
        }
  ...
  if(interaction.isButton() && interaction.message.author.id == client.user.id && interaction.customId.includes("PREMIUM-")){
    ...
    if(!guild) return interaction.reply("❌ **Меня выгнали из этого сервера**")
    ...
    if(requester){
      requester.send(`${interaction.customId == "PREMIUM-ACCEPT" ? `✅ **Ваш запрос: \`${guild.name}\` был одобрен!**` : `❌ **Ваш запрос: \`${guild.name}\` был откланён!**`}`).catch(()=>{});
    }
    if(interaction.customId == "PREMIUM-ACCEPT" && requester.id != guild.ownerId){
      guild.fetchOwner(owner => {
        owner.send(`✅ ** Ваш сервер: \`${guild.name}\` получил статус Премиума!**`).catch(() => {});
      }).catch(() => {});
    }
    if (interaction.customId == "PREMIUM-ACCEPT") {
      if (client.premium.get("global", "guilds").includes(guild.id)) {
        interaction.update({
          embeds: [interaction.message.embeds[0].setTitle(`✅ Сервер является Премиум участником!`)],
          components: []
        })
      } else {
        client.premium.push("global", guild.id, "guilds");
        interaction.update({
          embeds: [interaction.message.embeds[0].setTitle(`✅ Были приняты!`)],
          components: []
        })
      }
    } else {
      if (client.premium.get("global", "guilds").includes(guild.id)) {
        ...
        interaction.update({
          embeds: [interaction.message.embeds[0].setTitle(`❌ Были отказаны!`)],
          components: []
        })
      }
    }
